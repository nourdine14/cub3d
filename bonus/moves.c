...
/*   Updated: 2023/08/13 16:17:04 by oaboulgh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (info->player->move_forward_l && !has_wall2(info->player->x1 + dy, info->player->y1, info))
		info->player->x1 += dy;
	if (info->player->move_forward_l && !has_wall2(info->player->x1, info->player->y1 - dx, info))
		info->player->y1 -= dx;
	if (info->player->move_forward_r && !has_wall2(info->player->x1 - dy, info->player->y1, info))
		info->player->x1 -= dy;
	if (info->player->move_forward_r && !has_wall2(info->player->x1, info->player->y1 + dx, info))
		info->player->y1 += dx;
	if (info->player->move_forward && !has_wall2(info->player->x1 + dx, info->player->y1, info))
		info->player->x1 += dx;
	if (info->player->move_forward && !has_wall2(info->player->x1, info->player->y1 + dy, info))
		info->player->y1 += dy;
	if (info->player->move_backward && !has_wall2(info->player->x1 - dx, info->player->y1, info))
		info->player->x1 -= dx;
	if (info->player->move_backward && !has_wall2(info->player->x1, info->player->y1 - dy, info))
		info->player->y1 -= dy;
...
void move_player(t_player *player, t_cub *info)
{
	...
	if (player->shot)
	{
		info->gun1.img_ptr = mlx_xpm_file_to_image(info->mlx, "textures/gun2.xpm", &x1, &y1);
		free(info->gun1.addr);
...
void	full_images3(t_cub *info, int x, int y)
{
	...
	info->door.img_ptr = mlx_xpm_file_to_image(info->mlx, "textures/door_elon.xpm", &x, &y);
	...
	info->doorside.img_ptr = mlx_xpm_file_to_image(info->mlx, "textures/door_elon_side.xpm", &x, &y);
	if (!info->doorside.img_ptr)
	{
		printf("Error image couldn't load\n");
		exit (1);
	}
	info->doorside.addr = mlx_get_data_addr(info->doorside.img_ptr, &info->doorside.bpp, &info->doorside.line_len, &info->doorside.endian);
...
void	realloc_pos(char **map, int x, int y, char c)
{
	char	*str;
	int		i;
	...
	str = ft_strdup(map[x]);
	free(map[x]);
	map[x] = malloc(sizeof(char) * (ft_strlen(str) + 1));
	while (str[i])
	{
		if (i != y)
			map[x][i] = str[i];
...
void	check_doors_distance_from_player(t_cub *info)
{
	...
	i = 0;
	if (!info->doors_position)
		return ;
	while (i < info->num_of_doors)
	{
		if (diff_of_two_points(info->player->x - 25, info->player->y - 25, info->doors_position[i] * G_SIZE, \
			info->doors_position[i + 1] * G_SIZE) < 100)
		{
			x = info->doors_position[i];
			y = info->doors_position[i + 1];
			info->player->map[y][x] = 'C';
		}
		else
		{
			x = info->doors_position[i];
