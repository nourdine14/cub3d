...
/*   Updated: 2023/08/12 16:40:52 by oaboulgh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_doors_position(t_cub *info, char **map)
{
	int	i;
	int	j;
	int	num_of_doors;
	...
	info->doors_position = malloc(sizeof(int) * (num_of_doors));
	i = 0;
	num_of_doors = 0;
	while (map[i])
	{
		...
		while (map[i][j] != '\0')
		{
			if (map[i][j] == 'D')
			{
				info->doors_position[num_of_doors++] = j;
				info->doors_position[num_of_doors++] = i;
			}
			j++;
		}
		i++;
...
int	main(int ac, char **av)
{
	...
	info.player = &player;
	init_player(&player, map2);
	init_doors_position(&info, map2);
	info.mlx = mlx_init();
	if (info.mlx == NULL)
