...
/*   Updated: 2023/08/13 15:55:24 by oaboulgh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontale(t_cub *info, t_ray *ray, float ray_angle)
{
	...
	int	p = height_of_map(info->player->map);
	while (IX >= 0 && IX < k * G_SIZE && IY >= 0 && IY < p * G_SIZE)
	{
...
void	draw_wall(t_cub *info, t_ray *ray, float ray_angle, int i)
{
	...
	double	wall_projected = (25000 / ray->ray_dist);
	if (wall_projected > 3000)
		wall_projected = 3000;
